...
export default function Home() {
  ...
  async function handleReserve(inventoryId, warehouseName) {
    ...
    try {
      ...
      if (res.status === 409) {
        setError("Not enough stock available at " + warehouseName);
        setReserving(null);
        return;
      }
      if (!res.ok) {
                  ...
                  <div key={wh.inventoryId} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <span className={wh.available === 0 ? "text-xs font-medium px-2 py-0.5 rounded-full bg-red-100 text-red-700" : wh.available <= 3 ? "text-xs font-medium px-2 py-0.5 rounded-full bg-orange-100 text-orange-700" : "text-xs font-medium px-2 py-0.5 rounded-full bg-green-100 text-green-700"}>
                    ...
                    <button
                      onClick={() => handleReserve(wh.inventoryId, wh.warehouseName)}
                      disabled={wh.available === 0 || reserving === wh.inventoryId}
                      className={wh.available === 0 ? "px-4 py-2 rounded-lg text-sm font-medium bg-gray-100 text-gray-400 cursor-not-allowed" : reserving === wh.inventoryId ? "px-4 py-2 rounded-lg text-sm font-medium bg-blue-400 text-white cursor-not-allowed" : "px-4 py-2 rounded-lg text-sm font-medium bg-blue-600 text-white hover:bg-blue-700"}
                    >
